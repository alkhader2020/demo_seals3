...
const normalizeStatus = (s: unknown): TaskStatus => {
  if (s === "pending" || s === "in-progress" || s === "completed") return s;
  if (s === "待开始") return "pending";
  if (s === "进行中") return "in-progress";
  if (s === "已完成") return "completed";
  return "pending";
};

const normalizeType = (t: unknown): TaskType => {
  return t === "销售角色实训" ? "销售角色实训" : "销售基础训练";
};

const normalizeMode = (m: unknown): TaskMode | undefined => {
  if (m === "选择题模式" || m === "开放问答模式" || m === "自由对话练习") return m;
  return undefined;
};

const isEmployee = (e: any): e is Employee =>
  e && typeof e.id === "string" && typeof e.name === "string" && typeof e.department === "string";

const toTaskArray = (raw: any): Task[] => {
  if (!Array.isArray(raw)) return [];
  return raw.map((t: any): Task => ({
    id: String(t?.id ?? Date.now()),
    title: String(t?.title ?? ""),
    description: String(t?.description ?? ""),
    assignees: Array.isArray(t?.assignees) ? t.assignees.filter(isEmployee) : [],
    startDate: String(t?.startDate ?? ""),
    deadline: String(t?.deadline ?? ""),
    supervisor: String(t?.supervisor ?? ""),
    status: normalizeStatus(t?.status),
    createdAt: String(t?.createdAt ?? new Date().toISOString().split("T")[0]),
    type: normalizeType(t?.type),
    mode: normalizeMode(t?.mode),
    taskType: t?.taskType === "官方任务" ? "官方任务" : "自主任务",
  }));
};
...
export default function TaskManagementPage() {
  ...
  useEffect(() => {
    try {
      const savedOfficialRaw = JSON.parse(localStorage.getItem("tasksManagement") || "[]");
      const official = toTaskArray(savedOfficialRaw);
      if (official.length > 0) setTasks(official);
    } catch {}

    try {
      const savedAutonomousRaw = JSON.parse(localStorage.getItem("autonomousTasks") || "[]");
      const autonomous = toTaskArray(savedAutonomousRaw);
      if (autonomous.length > 0) setAutonomousTasks(autonomous);
    } catch {}
  }, []);
  ...
  const handleCreateAutonomousTask: React.FormEventHandler<HTMLFormElement> = (e) => {
    ...
    const newTask: Task = {
      ...
      type: normalizeType(fd.get("type")),
      taskType: "自主任务",
    };
  ...
  const startAutonomousTask = (taskId: string) => {
    ...
    const updated: Task[] = autonomousTasks.map((t): Task =>
    ...
    if (
      task &&
      (task.type === "销售角色实训" ||
        task.title?.includes("产品介绍") ||
        task.title?.includes("异议处理") ||
        task.title?.includes("价格谈判") ||
        task.title?.includes("客户跟进"))
    ) {
  ...
  const deleteAutonomousTask = (taskId: string) => {
    const updated: Task[] = autonomousTasks.filter((t): t is Task => t.id !== taskId);
    setAutonomousTasks(updated);
    localStorage.setItem("autonomousTasks", JSON.stringify(updated));
  };
                      ...
                      <tr key={task.id}>
                        ...
                        <td className="px-6 py-4">
                          <div className="flex flex-wrap gap-1">
                            {task.assignees.slice(0, 3).map((assignee: Employee) => (
                              <span
                                key={assignee.id}
                          ...
                          {task.status === "pending" && (
                            <button
                              ...
                              onClick={() => {
                                ...
                                } else if (task.type === "销售角色实训") {
                                  window.location.href = "/live-training";
                                } else {
                                  // 其他任务类型的处理逻辑
                                  alert("开始学习任务：" + task.title);
                                }
                              }}
                            >
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 border-t border-gray-200 flex justify-between">
              {(selectedTask.type === "销售角色实训" ||
                selectedTask.title?.includes("产品介绍") ||
                selectedTask.title?.includes("异议处理") ||
                selectedTask.title?.includes("价格谈判") ||
                selectedTask.title?.includes("客户跟进")) && (
                <button
                  onClick={() => {
              ...
              <button
                ...
                className="px-4 py-2 bg-emerald-500 text-white rounded-lg hover:bg-emerald-600 ml-auto"
              >
                关闭
              </button>
            </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-gray-200 flex justify-between items-center">
              ...
              <button onClick={() => setShowProductIntro(false)} className="text-gray-500 hover:text-gray-700">
                ×
              </button>
            </div>
            <div className="p-6">
            ...
            <div className="p-6 border-t border-gray-200 flex justify-between">
              ...
              <button
                onClick={() => setShowProductIntro(false)}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-gray-200 flex justify-between items-center">
              ...
              <button onClick={() => setShowScenarioPage(false)} className="text-gray-500 hover:text-gray-700">
                ×
              </button>
            </div>
            <div className="p-6">
